...
#$(expr ${network_capacity} \* 2 )
					...
					branch_rate_mid=$(expr $(expr $network_capacity \* 1) / 2)
					branch_rate_small=$(expr $(expr $network_capacity \* 1) / 3)
					...
					do
						...
						sleep 0.01
						echo "sudo tc class add dev $int2exp_sink parent 1: classid 1:3 htb rate ${branch_rate_mid}mbit ceil ${branch_rate_mid}mbit"
						ssh ${uname}@${host} -i $keyfile "sudo tc class add dev $int2exp_sink parent 1: classid 1:3 htb rate ${branch_rate_mid}mbit ceil ${branch_rate_mid}mbit" # limit $(expr ${branch_rate_mid} \* 500 )" # in bytes
						sleep 0.01
						echo "sudo tc qdisc add dev $int2exp_sink parent 1:3 bfifo limit $(expr $(expr $(expr ${branch_rate_mid} \* 250 ) \* $RTT_us ) / 1000 )"
						ssh ${uname}@${host} -i $keyfile "sudo tc qdisc add dev $int2exp_sink parent 1:3 bfifo limit $(expr $(expr $(expr ${branch_rate_mid} \* 250 ) \* $RTT_us ) / 1000 )" # 2*BDP of that link in in bytes
						sleep 0.01
						# logging
					...
					do
						...
						ssh ${uname}@${host} -i $keyfile "sudo tc qdisc add dev $int2exp_sink root handle 1: htb default 3"
						sleep 0.01
						echo "sudo tc class add dev $int2exp_sink parent 1: classid 1:3 htb rate ${branch_rate_small}mbit ceil ${branch_rate_small}mbit"
						ssh ${uname}@${host} -i $keyfile "sudo tc class add dev $int2exp_sink parent 1: classid 1:3 htb rate ${branch_rate_small}mbit ceil ${branch_rate_small}mbit" # limit $(expr ${branch_rate} \* 500 )" # in bytes
						sleep 0.01
						echo "sudo tc qdisc add dev $int2exp_sink parent 1:3 bfifo limit $(expr $(expr $(expr ${branch_rate_small} \* 250 ) \* $RTT_us ) / 1000 )"
						ssh ${uname}@${host} -i $keyfile "sudo tc qdisc add dev $int2exp_sink parent 1:3 bfifo limit $(expr $(expr $(expr ${branch_rate_small} \* 250 ) \* $RTT_us ) / 1000 )" # 2*BDP of that link in in bytes
						sleep 0.01
					...
					do
						...
						sleep 0.01
						echo "sudo tc class add dev $int2exp_sink parent 1: classid 1:3 htb rate ${branch_rate_small}mbit ceil ${branch_rate_small}mbit"
						ssh ${uname}@${host} -i $keyfile "sudo tc class add dev $int2exp_sink parent 1: classid 1:3 htb rate ${branch_rate_small}mbit ceil ${branch_rate_small}mbit" # limit $(expr ${branch_rate_small} \* 500 )" # in bytes
						sleep 0.01
						echo "sudo tc qdisc add dev $int2exp_sink parent 1:3 bfifo limit $(expr $(expr $(expr ${branch_rate_small} \* 250 ) \* $RTT_us ) / 1000 )"
						ssh ${uname}@${host} -i $keyfile "sudo tc qdisc add dev $int2exp_sink parent 1:3 bfifo limit $(expr $(expr $(expr ${branch_rate_small} \* 250 ) \* $RTT_us ) / 1000 )" # 2*BDP of that link in in bytes
						sleep 0.01
