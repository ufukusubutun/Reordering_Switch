...
while [ 1 ]; do 
	...
	# to capture in units of packets 
	tc -s -d qdisc show dev $(ip route get 10.14.1.2 | grep -oP "(?<= dev )[^ ]+") | grep -A2 -E "$grep_str" | grep backlog | awk -F" " '{print $3}' | sed 's/p//g' | paste -s -d, - | tee -a $filename
